...
| 5 | **Browser client + QR pairing** | The `term.srcfl.xyz` SPA: passkey + `prf`, `RTCPeerConnection` + Noise-JS + xterm; plus the interactive **token/QR pairing** (NNpsk0) co-developed here (the browser is pairing's other end). | next | Real iPhone Safari: passkey once, scan a QR to pair, attach to a live shell over P2P. |
