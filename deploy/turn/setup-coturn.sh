...
# static-auth-secret into /etc/tr-signal.env so tr-signal can mint ephemeral
...
ENVFILE=/etc/tr-signal.env

# Generate (or reuse) the shared secret. tr-signal reads it from $ENVFILE.
if [ -f "$ENVFILE" ] && grep -q '^TR_TURN_SECRET=' "$ENVFILE"; then
  SECRET="$(grep '^TR_TURN_SECRET=' "$ENVFILE" | cut -d= -f2-)"
else
  SECRET="$(openssl rand -hex 32)"
  printf 'TR_TURN_SECRET=%s\n' "$SECRET" > "$ENVFILE"
  chmod 600 "$ENVFILE"
fi
...
echo "shared secret in $ENVFILE — restart tr-signal with --turn-url turn:$REALM:3478"
